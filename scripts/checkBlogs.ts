...
(async () => {
  ...
  const blogs = await p.blogPost.findMany({
    ...
    take: 5,
    include: { connectedWebsite: { select: { url: true, niche: true } } }
  });
  for (const b of blogs) {
    ...
    console.log('---');
    console.log('Site:', b.connectedWebsite?.url);
    console.log('Niche:', b.connectedWebsite?.niche);
    ...
    console.log('SEO Score:', b.seoScore);
  }
  await p.$disconnect();
})();
