// One-off backfill: set featuredImage on published BlogPosts that lack one,
// so every live/embedded blog card shows a real niche-matched photo.
...
(async () => {
  const rows = await prisma.blogPost.findMany({
    ...
    include: { schedules: { include: { subscription: { include: { connectedWebsite: true } } } } },
  });
  console.log('Published posts without featured image:', rows.length);
  for (const p of rows) {
    const niche = (p.schedules && p.schedules[0] && p.schedules[0].subscription && p.schedules[0].subscription.connectedWebsite && p.schedules[0].subscription.connectedWebsite.niche) || undefined;
    const url = await getBlogImage(p.title, niche);
    await prisma.blogPost.update({ where: { id: p.id }, data: { featuredImage: url } });
    console.log('  ✔', p.title.slice(0, 60));
  }
  await prisma.$disconnect();
  console.log('Post image backfill done.');
})();
