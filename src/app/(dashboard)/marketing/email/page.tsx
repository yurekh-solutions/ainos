...
import {
  ...
  Hash, Zap, Clock, TrendingUp, Loader2, X, RefreshCw
} from 'lucide-react';
...
export default function SocialMediaPage() {
  ...
  const handleGenerate = async () => {
    if (!topic.trim() && !videoDescription.trim()) {
      showToast('Please enter a topic or video description', 'error');
      return;
    }
    setGenerating(true);
    setGenerated(null);
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-sm mt-0.5 text-gray-600 dark:text-gray-400">Upload a reel. Get platform-optimized captions, hooks & hashtags for every platform — in seconds.</p>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5 flex items-center gap-1.5">
                  <Video className="w-3.5 h-3.5 text-purple-500" /> What&apos;s your reel about? *
                </label>
                <input
                  value={topic}
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5 flex items-center gap-1.5">
                  <Upload className="w-3.5 h-3.5 text-purple-500" /> Video Description (optional)
                </label>
                <textarea
                  value={videoDescription}
                  onChange={e => setVideoDescription(e.target.value)}
                  placeholder="Describe what happens in your video for more accurate captions..."
                  rows={3}
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2 flex items-center gap-1.5">
                <Hash className="w-3.5 h-3.5 text-purple-500" /> Select Platforms
              </label>
          ...
          <div className="mt-6 pt-4 border-t border-gray-100 dark:border-gray-800">
            <button
              ...
              disabled={generating || (!topic.trim() && !videoDescription.trim())}
            ...
            >
              {generating ? (
                <>
                  ...
                  AI is crafting perfect captions for all platforms...
                </>
              ) : (
                <>
                  <Sparkles className="w-4 h-4" /> Generate Platform-Optimized Captions
                </>
              )}
            </button>
          </div>
        ...
        {generating && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-sm font-semibold text-gray-900 dark:text-white">AI Caption Agent is working...</p>
                <p className="text-xs text-gray-500">Analyzing your content and crafting platform-specific captions</p>
              </div>
            </div>
            <div className="space-y-2">
              {['Analyzing video content & topic...', 'Researching trending hooks for each platform...', 'Writing optimized captions with CTAs...', 'Generating relevant hashtags...', 'Finalizing platform-specific formatting...'].map((step, i) => (
                <motion.div
                  key={step}
              ...
              {generated.generalTips.length > 0 && (
                <div className="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-2xl border border-amber-200 dark:border-amber-800 p-5 mb-6">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-sm font-bold text-amber-900 dark:text-amber-200">Pro Tips for Maximum Reach</h3>
                  </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex items-center gap-1.5">
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${
                            p.characterCount <= config.maxChars
                          ...
                          <button
                            onClick={() => handleCopy(p.platform, fullCaption)}
                      ...
                      {p.hooks.length > 0 && (
                        <div className="mb-3">
                          ...
                          <div className="space-y-1">
                            {p.hooks.map((hook, j) => (
                              <p key={j} className="text-xs text-gray-700 dark:text-gray-300 bg-white/50 dark:bg-gray-800/50 rounded-lg px-3 py-1.5">
                                {hook}
                              </p>
                            ))}
                          </div>
                      ...
                      <div className="mb-3">
                        ...
                        <p className="text-xs text-gray-700 dark:text-gray-300 bg-white/50 dark:bg-gray-800/50 rounded-lg px-3 py-2 leading-relaxed whitespace-pre-wrap">
                          {p.caption}
                        </p>
                      </div>
                      ...
                      {p.hashtags.length > 0 && (
                        <div>
                          <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Hashtags</p>
                          <div className="flex flex-wrap gap-1">
                            {p.hashtags.map((tag, j) => (
                              <span key={j} className="px-2 py-0.5 rounded-md text-[10px] bg-white/50 dark:bg-gray-800/50 text-gray-600 dark:text-gray-400 font-medium">
                                {tag}
                              </span>
                            ))}
                          </div>
        ...
        {!generated && !generating && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <p className="text-sm text-gray-500 max-w-md mx-auto">
              Enter your reel topic above and get AI-powered captions optimized for Instagram, TikTok, YouTube Shorts, LinkedIn, X, and Facebook.
            </p>
